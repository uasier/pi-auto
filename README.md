...
左边选会话，中间看终端并发送，右边排计划和循环。

- **发送**：输入框写内容，点「发送」，或 ⌘ / Ctrl + Enter。优先走 Herdr `agent.prompt`，不行再向 pane 输入。
- **计划**：把输入加入计划，或导入文本。计划只属于当前选中的窗口。
...
中间是一局贪吃蛇。手动、Jev、Laya 都要点「开始」才走。Jev / Laya 只在决策返回后移动，不会自己定时走。蛇头碰到墙或任意一节蛇身都算失败。决策日志在棋盘右侧。
